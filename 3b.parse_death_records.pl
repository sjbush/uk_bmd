...
df_b<-read.table('C:/Users/User/Desktop/bmd_as_resource/dataset_B/summary_of_records_per_year.txt',sep='\t',quote='',header=T)
df_d<-read.table('C:/Users/User/Desktop/bmd_as_resource/dataset_D/summary_of_records_per_year.txt',sep='\t',quote='',header=T)
fig1 <- ggplot() + geom_line(data = df_b, aes(x = Year, y = No..of.records.with.a.forename), linewidth = 0.7, linetype = 'solid') + geom_line(data = df_d, aes(x = Year, y = No..of.records.with.a.forename), linewidth = 0.7, linetype = 'dashed') + xlab('Year') + ylab('No. of records (thousands)') + scale_x_continuous(limits=c(1730,2020)) + scale_y_continuous(expand=c(0,5),limits=c(0,255000),breaks=c(0,50000,100000,150000,200000,250000),labels=c(0,50,100,150,200,250)) + theme_bw()
ggsave(file = 'C:/Users/User/Desktop/bmd_as_resource/Documents/Figure1.jpeg', plot = fig1, width = 7, height = 7)
...
df<-df.sub
fig4a <- ggplot() + geom_line(data = df, aes(x = Year, y = Gender.ratio.of..male.mostly.male...female...mostly.female..forename.records), linewidth = 0.7) + ggtitle('A') + xlab('Year') + ylab('Ratio of male to female birth records') + scale_x_continuous(limits=c(1830,2020)) + scale_y_continuous(limits=c(0.95,1.1)) + theme_bw() + annotate("rect", xmin = 1830, xmax = 2020, ymin = 1.03, ymax = 1.06, alpha = 0.3, fill = "grey")

# plot the gender ratio of death records per year

df<-read.table('C:/Users/User/Desktop/bmd_as_resource/dataset_D/summary_of_records_per_year.txt',sep='\t',quote='',header=T)
df.sub<-subset(df,df$No..of.records.with.a.forename > 1000)
df<-df.sub
fig4b <- ggplot() + geom_line(data = df, aes(x = Year, y = Gender.ratio.of..male.mostly.male...female...mostly.female..forename.records), linewidth = 0.7) + ggtitle('B') + xlab('Year') + ylab('Ratio of male to female death records') + scale_x_continuous(limits=c(1730,2009)) + scale_y_continuous(limits=c(0.7,1.7)) + theme_bw() + annotate("rect", xmin = 1730, xmax = 2009, ymin = 1.03, ymax = 1.06, alpha = 0.3, fill = "grey")

# combine the two plots

fig4<-ggarrange(fig4a, fig4b, nrow=1, ncol=2)
ggsave(file = 'C:/Users/User/Desktop/bmd_as_resource/Documents/Figure4.jpeg', plot=fig4, width = 14, height = 7)

# plot the % of death records with a forename of undetermined gender

df<-read.table('C:/Users/User/Desktop/bmd_as_resource/dataset_D/summary_of_records_per_year.txt',sep='\t',quote='',header=T)
fig5b <- ggplot() + geom_line(data = df, aes(x = Year, y = X..of.records.with.a.gender.undetermined.forename), linewidth = 0.7) + xlab('Year') + ylab('% of death records with a gender-undetermined forename') + scale_x_continuous(limits=c(1830,2020)) + theme_bw()
...
# REQUIREMENTS
my $in_dir = 'BMD-13.09.23/D'; # manually downloaded
...
for(my $x=0;$x<=1;$x++)
	{ my %names = (); my %data = (); my %female = (); my %male = (); my $data_type = '';
	  ...
	  while((my $name,my $irrel)=each(%names))
		{ push(@names,$name); }
	  my @sorted_names = sort {"\L$a" cmp "\L$b"} @names;
