# PURPOSE: predict the gender of a given name on the basis of US SSA data, following the method of http://www.digitalhumanities.org/dhq/vol/9/3/000223/000223.html and mindful that it is, to quote that paper, a "blunt tool to study a complex subject" (see also https://github.com/lmullen/gender)

use strict;
use warnings;

# REQUIREMENTS
my $in_dir  = 'us_ssa'; # accurate as of 7th June 2023. From https://www.ssa.gov/OACT/babynames/names.zip | https://www.ssa.gov/OACT/babynames/limits.html
if (!(-d($in_dir))) { print "ERROR: unable to find $in_dir\n"; exit 1; }

# OUTPUT
my $out_file1 = 'gender_breakdown_per_year_of_names_in_US_SSA.txt';
my $out_file2 = 'gender_prediction_of_names_in_US_SSA.txt';
...
foreach my $file (@files)
	{ next if (($file eq '.') or ($file eq '..'));
	  next if ($file !~ /\.txt$/);
	  my $year;
	  if ($file =~ /^yob(\d+)\.txt$/) { $year = $1; }
	  $years{$year}++;
	  open(IN,"$in_dir/$file") or die $!;
	  while(<IN>)
		{ my $line = $_; chomp($line); $line =~ s/[\r\n]//g;
		  my @line = split(/\,/,$line);
		  my $name = $line[0]; my $sex = $line[1]; my $count = $line[2];
		  if 	($sex eq 'F') { $female{$name}{$year}+= $count; }
		  elsif ($sex eq 'M') { $male{$name}{$year}+= $count;   }
		  $names{$name}++;
		}
	  close(IN) or die $!;
	}
