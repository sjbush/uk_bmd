...
foreach my $region (@sorted_regions)
	{ next if (($region eq '.') or ($region eq '..'));
	  ...
	  elsif ($region eq 'Berkshire')  	 { $url = 'www.berkshirebmd.org.uk';     $date_of_last_update = '27th October 2011';   }
	  elsif ($region eq 'Cheshire')   	 { $url = 'www.cheshirebmd.org.uk';      $date_of_last_update = '6th October 2021';    }	  
	  elsif ($region eq 'Cumbria') 	  	 { $url = 'www.cumbriabmd.org.uk';       $date_of_last_update = '12th December 2011';  }
	  elsif ($region eq 'Kingston') 	 { $url = 'www.kingstonbmd.org.uk/'; 	 $date_of_last_update = '27th April 2023';     }	  
	  elsif ($region eq 'Lancashire') 	 { $url = 'www.lancashirebmd.org.uk'; 	 $date_of_last_update = '13th April 2023';     }
	  elsif ($region eq 'North_Wales') 	 { $url = 'www.northwalesbmd.org.uk'; 	 $date_of_last_update = '21st July 2012'; 	   }
	  elsif ($region eq 'Shropshire') 	 { $url = 'www.shropshirebmd.info'; 	 $date_of_last_update = '30th April 2020'; 	   }
	  elsif ($region eq 'Staffordshire') { $url = 'www.staffordshirebmd.org.uk'; $date_of_last_update = '8th June 2023';       }
	  elsif ($region eq 'West_Midlands') { $url = 'www.westmidlandsbmd.org.uk';  $date_of_last_update = '3rd June 2016'; 	   }
	  elsif ($region eq 'Wiltshire') 	 { $url = 'www.wiltshirebmd.org.uk'; 	 $date_of_last_update = '9th February 2018';   }
	  elsif ($region eq 'Yorkshire') 	 { $url = 'www.yorkshirebmd.org.uk'; 	 $date_of_last_update = '23rd February 2015';  }
...
while((my $year,my $irrel)=each(%births_per_year))
	{ my $total_num_of_births_this_year = $births_per_year{$year};
	  push(@years,$year);
	  $usable_years{$year}++;
	}
...
print OUT_PCT_FIRST "Year of abandonment (year in which the proportion of births with that name first drops below $abandon_pc% of its past maximum; given only if applicable and for names used (a) a total of > $min_births times, and (b) in consecutive use for >= $min_consecutive_years years, with (c) their peak usage within the longest consecutive use period)\t";
print OUT_PCT_FIRST "Year of revival (after abandonment, year in which the proportion of births with that name reaches $revival_pc% of its past maximum; given only if applicable and for names used (a) a total of > $min_births times, and (b) in consecutive use for >= $min_consecutive_years years, with (c) their peak usage, year of abandonment and year of revival within the longest consecutive use period, and (d) at least $years_between_abandonment_and_revival years between abandonment and revival)\t";
...
print OUT_ABS_FIRST "Year of abandonment (year in which the proportion of births with that name first drops below $abandon_pc% of its past maximum; given only if applicable and for names used (a) a total of > $min_births times, and (b) in consecutive use for >= $min_consecutive_years years, with (c) their peak usage within the longest consecutive use period)\t";
print OUT_ABS_FIRST "Year of revival (after abandonment, year in which the proportion of births with that name reaches $revival_pc% of its past maximum; given only if applicable and for names used (a) a total of > $min_births times, and (b) in consecutive use for >= $min_consecutive_years years, with (c) their peak usage, year of abandonment and year of revival within the longest consecutive use period, and (d) at least $years_between_abandonment_and_revival years between abandonment and revival)\t";
...
print OUT_PCT_MIDDLE "Year of abandonment (year in which the proportion of births with that name first drops below $abandon_pc% of its past maximum; given only if applicable and for names used (a) a total of > $min_births times, and (b) in consecutive use for >= $min_consecutive_years years, with (c) their peak usage within the longest consecutive use period)\t";
print OUT_PCT_MIDDLE "Year of revival (after abandonment, year in which the proportion of births with that name reaches $revival_pc% of its past maximum; given only if applicable and for names used (a) a total of > $min_births times, and (b) in consecutive use for >= $min_consecutive_years years, with (c) their peak usage, year of abandonment and year of revival within the longest consecutive use period, and (d) at least $years_between_abandonment_and_revival years between abandonment and revival)\t";
...
print OUT_ABS_MIDDLE "Year of abandonment (year in which the proportion of births with that name first drops below $abandon_pc% of its past maximum; given only if applicable and for names used (a) a total of > $min_births times, and (b) in consecutive use for >= $min_consecutive_years years, with (c) their peak usage within the longest consecutive use period)\t";
print OUT_ABS_MIDDLE "Year of revival (after abandonment, year in which the proportion of births with that name reaches $revival_pc% of its past maximum; given only if applicable and for names used (a) a total of > $min_births times, and (b) in consecutive use for >= $min_consecutive_years years, with (c) their peak usage, year of abandonment and year of revival within the longest consecutive use period, and (d) at least $years_between_abandonment_and_revival years between abandonment and revival)\t";
	...
	{ my %names = (); my %data = (); my %female = (); my %male = (); my $data_type = '';
	  ...
	  foreach my $name (@sorted_names)
		{ #print "parsing $data_type, first round: $name...\n";
		  ...
		  my $total_F_births_with_this_name = 0; if (exists($female{$name})) { $total_F_births_with_this_name = $female{$name}; }
		  my $total_M_births_with_this_name = 0; if (exists($male{$name}))   { $total_M_births_with_this_name = $male{$name};   }	  
		  if ($name ne "(none)")
			{ push(@all_name_usage,[$total_number_of_people_with_this_name_across_time,$name]);
			  push(@all_name_usage_F,[$total_F_births_with_this_name,$name]) unless ($total_F_births_with_this_name == 0);
			  push(@all_name_usage_M,[$total_M_births_with_this_name,$name]) unless ($total_M_births_with_this_name == 0);
			}
		}
		...
		{ #print "parsing $data_type, second round: $name...\n";
		  ...
		  foreach my $year (@sorted_years)
			{ my $total_num_of_births_this_year = $births_per_year{$year};
			  if (exists($data{$year}{$name}))
				{ my $num_names = $data{$year}{$name};
				  my $pc = sprintf("%.6f",(($num_names/$total_num_of_births_this_year)*100));
				  push(@arr,[$pc,$year]);
				}
			  else
				{ push(@arr,[0,$year]); }
		  ...
		  my $min_no_of_births 	  = $sorted_out_line_num[0];
		  my $max_no_of_births    = $sorted_out_line_num[$#sorted_out_line_num];
		  my $max_pc_of_births	  = $sorted_out_line_pc[$#sorted_out_line_pc];
		  my $pc_of_max_abandon	  = $abandon_frac*$max_pc_of_births;
		  my $pc_of_max_revival	  = $revival_frac*$max_pc_of_births;
		  ...
		  my $consecutive_period_end   			  = $sorted_len_of_consec_years[0][2];
	  	  ...
	  	  # for names used > $min_births times, for longer than $min_consecutive_years consecutive years and with peak usage within this period, determine the year of abandonment (if applicable)
		  my $year_of_abandonment = 'NA';
		  my $first_peak_year = $peak_year;
		  if (($peak_year =~ /^(\d+)\-.*?$/) or ($peak_year =~ /^(\d+)\,.*?$/)) { $first_peak_year = $1; }
		  my $absolute_number_of_births_with_this_name = sum(@out_line_num);
		  if (($max_num_of_consecutive_years_of_use >= $min_consecutive_years) and ($absolute_number_of_births_with_this_name > $min_births) and ($first_peak_year > $consecutive_period_start) and ($first_peak_year < $consecutive_period_end))
			{ for(my $x=0;$x<@sorted_years;$x++)
				{ my $yr = $sorted_years[$x];
				  my $pc = $out_line_pc[$x];
		  ...
		  if ($x == 0)
			{ print OUT_PCT_FIRST "$name\t$gender\t$rank_all\t$rank_f\t$rank_m\t$num_of_years_in_use\t$max_num_of_consecutive_years_of_use\t$absolute_number_of_births_with_this_name\t$num_occurrences_as_forename\t$num_occurrences_as_middle_name\t$ratio_of_forename_to_middle_name_use\t$min_no_of_births\t$max_no_of_births\t$first_recorded_sighting\t$peak_year\t$first_peak_year\t$last_recorded_sighting\t$debut_year\t$year_of_abandonment\t$year_of_revival\t$num_of_years_debut_to_peak\t$num_of_years_peak_to_abandonment\t$num_of_years_debut_to_abandonment\t$num_of_years_abandonment_to_revival\t$out_line_pc\n";
...
for(my $y=0;$y<@sorted_years;$y++)
	{ my $year = $sorted_years[$y];
	  ...
	  my $avg_no_of_middle_names_per_record = avg(@{$average_number_of_middle_names_per_record{$year}});
