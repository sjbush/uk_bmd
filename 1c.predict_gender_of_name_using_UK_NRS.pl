# PURPOSE: predict the gender of a given name on the basis of UK NRS (National Records of Scotland) data, following the method of http://www.digitalhumanities.org/dhq/vol/9/3/000223/000223.html and mindful that it is, to quote that paper, a "blunt tool to study a complex subject" (see also https://github.com/lmullen/gender)

use strict;
use warnings;

# REQUIREMENTS
my $in_file = 'uk_nrs/1974-2022.txt'; # manually created using data accurate as of 14th June 2023. From https://www.nrscotland.gov.uk/statistics-and-data/statistics/statistics-by-theme/vital-events/names/babies-first-names
if (!(-e($in_file))) { print "ERROR: unable to find $in_file\n"; exit 1; }

# OUTPUT
my $out_file1 = 'gender_breakdown_per_year_of_names_in_UK_NRS.txt';
my $out_file2 = 'gender_prediction_of_names_in_UK_NRS.txt';
