# PURPOSE: predict the gender of a given name on the basis of UK ONS data, following the method of http://www.digitalhumanities.org/dhq/vol/9/3/000223/000223.html and mindful that it is, to quote that paper, a "blunt tool to study a complex subject" (see also https://github.com/lmullen/gender)
...
# OUTPUT
my $out_file1 = 'gender_breakdown_per_year_of_names_in_UK_ONS.txt';
my $out_file2 = 'gender_prediction_of_names_in_UK_ONS.txt';
...
my %names = (); my %years = ();
my %female = (); my %male = ();
my %headers = (); my %names_per_gender_per_year = ();
	  ...
	  for(my $x=2;$x<@line;$x++)
		{ my $year  = $headers{$x};
		  ...
		  if (exists($names_per_gender_per_year{$name}{$gender}{$year}))
			{ print "error in $in_file1: we already have a record for $name in year $year, i.e. there is a duplicate entry at line $.\n"; exit 1; }
...
while(<IN>)
	{ next if ($. == 1);
	  ...
	  $name = uc($name);
	  if (exists($names_per_gender_per_year{$name}{$gender}{$year}))
		{ print "error in $in_file2: we already have a record for $name in year $year, i.e. there is a duplicate entry at line $.\n"; exit 1; }
	  $names_per_gender_per_year{$name}{$gender}{$year}++;
	  if 	($gender eq 'F') { $female{$name}{$year}+= $count; }
	  elsif ($gender eq 'M') { $male{$name}{$year}+= $count;   }
	  $names{$name}++; $years{$year}++;
