...
**IMPORTANT NOTE 2**: as the UK BMD is an actively updated resource, publications making use of it have used incrementally revised versions. This repo archives the data associated with [Bush 2024](https://doi.org/10.5195/names.2024.2543) but this is NOT the latest version of the UK BMD. Its update history, including a link to the latest version, is as follows:
...
2. 23,468,892 birth records as of 29th January 2018; as used by both [Bush 2019](https://doi.org/10.1080/00277738.2018.1536186) and [Bush 2020](https://doi.org/10.1080/00277738.2020.1775471), with the processed dataset hosted via the [University of Edinburgh DataShare portal](http://dx.doi.org/10.7488/ds/2294).
3. 25,213,860 birth and 9,887,244 death records as of 13th September 2023; as used by [Bush 2024](https://doi.org/10.5195/names.2024.2543), with the processed datasets available in [this very repo](https://www.github.com/sjbush).
4. 24,084,977 birth and 9,888,837 death records as of 13th January 2026; as used by [Bush 2026](https://doi.org/10.5195/names.2026.2884), with the processed datasets hosted via [FigShare](https://doi.org/10.6084/m9.figshare.31249012).
...
The method for this is detailed [below](https://github.com/sjbush/uk_bmd/tree/main#criteria-for-exclusion) and described in [Bush 2024](https://doi.org/10.5195/names.2024.2543) as follows:
>"To avoid overcounting the same name, we also required that there be only one record per reference number. Multiple records with the same reference typically (but not always) denote variants of the same name, such as those without clear distinctions between the ‘forenames’ and ‘surname’ fields (for example, a pair of records with the names Tyler Van Der KAMP and Tyler VANDERKAMP, where the surname field is capitalised). However, this was only the case for Bath, Cumbria, Shropshire, the West Midlands, Wiltshire, and Yorkshire. In the other regions, reference numbers could not be used as they were not individual; rather, they referred to a processing batch of generally 5–10 people at a time."

To do so, I first counted the number of different reference numbers associated with each record. On line 197 of the [original code](https://github.com/sjbush/uk_bmd/blob/main/3a.parse_birth_records.pl), I checked whether there was only 1 reference number associated with that record and if there was, excluded all others. This was a mistake: the correct test was to check whether there was 1 _or more_ reference numbers associated with that record and if there was, to exclude all others. Thus, in the original code, a small number of birth records with >1 reference number (which in these six regions, often denote variants of the same name) were mistakenly being included in the final dataset, thus double-counting certain names. As multiple birth records often indicate a range of possibilities for the "mother's maiden name" field (discussed below), this issue of multiple copies does not apply to the death records, as they do not contain this field.
I apologise for the error (a _== 1_ instead of _>= 1_ in the code!) but emphasise that it affects only a small proportion of the total (<5% of birth records, principally those from Yorkshire) and that it does not materially alter any of the conclusions drawn in previously published work. Indeed, the BMD data analysed in [Bush 2019](https://doi.org/10.1080/00277738.2018.1536186) and [Bush 2023](https://doi.org/10.5195/names.2023.2500), for example, did not even use the full set of birth records but instead cross-referenced them with marriage and death records to create conservative subsets; in this respect, double-counted records would have been filtered out prior to any analysis. Moreover, while the combined set of birth records (regrettably inclusive of this small proportion of double-counted records) were analysed in [Bush 2020](https://doi.org/10.1080/00277738.2020.1775471) to chart patterns of alliterative name use over time (shown in Figure 3 of the paper), the analysis was also repeated for subsets of the data, including for regions where reference numbers were indeed unique; importantly, this resulted in "no observable difference in the trends illustrated in Figure 3".
...
Sanity tests of the data are performed both internally (by comparing subsets of the data with each other) and externally (by comparing the data to an outside source) using [Pearson correlations](https://www.biostathandbook.com/linearregression.html). More specifically:
