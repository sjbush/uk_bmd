...
To do so, I first counted the number of different reference numbers associated with each record. On line 197 of the [original code](https://github.com/sjbush/uk_bmd/blob/main/3a.parse_birth_records.pl), I checked whether there was only 1 reference number associated with that record and if there was, excluded all others. This was a mistake: the correct test was to check whether there was 1 _or more_ reference numbers associated with that record and if there was, to exclude all others. Thus, in the original code, a small number of records with >1 reference number (which in these six regions, often denote variants of the same name) were mistakenly being included in the final dataset, thus double-counting certain names.
I apologise for the error (a _== 1_ instead of _>= 1_ in the code) but emphasise that it affects only a small proportion of the total (<5% of birth records, principally those from Yorkshire) and that it does not materially alter any of the conclusions drawn in previously published work.
